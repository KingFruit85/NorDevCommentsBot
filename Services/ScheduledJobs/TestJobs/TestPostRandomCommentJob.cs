        ...
        try
        {
            ...
            foreach (var guild in client.Guilds)
            {
                ...
                try
                {
                    ...
                    logger.LogInformation("attempting to save message {msgId} to the RandomAlreadyPosted table", parsedMessageId);
                    _ = apiService.AddAlreadyPostedForGuild(parsedMessageIdValue, guild.Id);
                }
                catch (Exception e)
                {
                    logger.LogError("error when attempting to save to the RandomAlreadyPosted table: {error}", e.Message);
                }
            }
        }
        catch (Exception ex)
